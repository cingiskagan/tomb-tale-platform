...
# ── File editing ───────────────────────────────────────────────────────────
# Both helpers here write to a temp file and only overwrite the target once
# the filter has succeeded, so an edit that dies halfway cannot truncate the
# file. They copy the temp file through the original rather than moving it
# into place, so the mode and owner survive — .env holds secrets and its
# permissions are not ours to change.
#
# sed -i would be neither: it is not portable (BSD sed wants a suffix) and it
# edits in place.
sed_inplace() {
